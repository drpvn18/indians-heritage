    ...
    const getDiscountedPrice = (price, discount) => {
        if (discount) {
            if (discount?.type === "percentage") {
                return (parseFloat(price) - (price * (parseFloat(discount?.value) || 0) / 100))?.toFixed(2);
            } else if (discount?.type === "amount") {
                return (parseFloat(price) - parseFloat(discount?.value) || 0)?.toFixed(2);
            } else {
                        ...
                        (productsList?.length === 0) ? (
                            <div className="flex flex-nowrap justify-center items-center gap-[10px] my-[15%] py-[10px] w-full">
                                <div className="text-center">
                                    ...
                                    <p className="text-gray-500 text-[18px] py-[2px]">
                                        Add products to unlock free delivery
                                    </p>
                                </div>
                            </div>
                        ) : (
