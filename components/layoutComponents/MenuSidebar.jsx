...
export default function MenuSidebar({ openSidebar, setOpenSidebar }) {
    ...
    const menuItems = [
        ...
        },
        {
            ...
            "endpoint": "https://www.indianheritage.eu/indiankitchen/",
            "target": "_blank",
            "icon": <FoodTruck height={32} width={32} />
        },
    ];
                            ...
                            return (
                                <div key={item?.id}>
                                    <div onClick={() => setActiveCategory(activeCategory === item?.id ? "" : item?.id)}
                                        ...
                                        <Link href={`${item?.endpoint}`} target={`${item?.target === '_blank' ? '_blank' : '_self'}`} className='flex justify-start items-center gap-[5px]'>
                                            {item?.title}
                                            {item?.icon ? item?.icon : ""}
                                        </Link>
                                        {
