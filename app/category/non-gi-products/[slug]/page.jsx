...
export default function ProductCategory({ params }) {
    ...
    useEffect(() => {
        ...
        document.title = `${pageTitle} | Non-GI products - Indian Heritage | Europe's first Indian GI tagged & organic store`;
    ...
    useEffect(() => {
        try {
            ...
            if (slug === "all") {
                ...
                temp = products.filter((item) => item.product_type?.slug === "non-gi-products" && (item.category?.id?.includes(slug) || item.category?.main_category?.id?.includes(slug)));
            }
