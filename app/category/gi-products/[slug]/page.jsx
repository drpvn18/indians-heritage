...
export default function ProductCategory() {
    ...
    const checkForCategoriesToInclude = async (slug) => {
        let temp_categories = [];
        ...
        temp?.map((sub_category) => {
            if (sub_category?.parent_category?.slug === slug && sub_category?.product_type?.value === "gi") {
                temp_categories.push(sub_category?.slug);
            }
        });
    ...
    useEffect(() => {
        const handleFilterProducts = async () => {
            try {
                ...
                const categories_to_include = await checkForCategoriesToInclude(slug);

                if (categories_to_include?.length > 0) {
                    temp = temp.filter((item) => categories_to_include?.includes(item.category?.id));
                } else {
                    temp = temp.filter((item) => item.category?.id?.includes(slug));
                }
    ...
    const handleStateChange = (selectedOption) => {
        ...
        router.push(`/category/gi-products/${slug}?state=${selectedOption?.value}`);
    };
                                    ...
                                    productsList?.length === 0 ? (
                                        <div className="text-center py-[100px]">
                                            <p className="p-[10px] tracking-wide text-[20px]">
                                                No products found from this category.
                                            </p>
                                        </div>
                                    ) : (
