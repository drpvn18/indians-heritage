...
import { CircleChevronDown, Loader, Minus, Plus, ShoppingCart } from "lucide-react";
...
export default function Product() {
    ...
    const [productDetailsFetchingStatus, setProductDetailsFetchingStatus] = useState(true);
    const router = useRouter();

    useEffect(() => {
        document.title = `${productDetails?.name ? productDetails?.name + " - " : ""} Indian Heritage | Europe's first Indian GI tagged & organic store`;
    }, [productDetails]);
    ...
    useEffect(() => {
        ...
        let pid = window.location.href?.split("?pid=")[1];
        pid && fetchProducts(pid);
    }, []);
    ...
    useEffect(() => {
        ...
        if (Object.keys(productDetails)?.length !== 0 && productDetails?.images && productDetails?.images?.length > 0)
            setMainImage(productDetails?.images[0]);
    }, [productDetails]);

    useEffect(() => {
        if (productDetails?.category?.id) {
            const temp = products?.filter((item) => item?.category?.id === productDetails?.category?.id);
                    ...
                    productDetailsFetchingStatus ? (
                        ...
                        <div>
                                    ...
                                    <div className={styles.mainImage}>
                                        {
                                            mainImage?.url && (
                                                <div className="relative">
                                                    <Image src={mainImage?.url} alt={"main image"} width="350" height="200" priority={true} />
                                                    {
                                ...
                                <div className={styles.product_details}>
                                    ...
                                    </div>
                                    {
                                        productDetails?.status !== "Active" ? (
                                            <div className={`w-fit rounded-full py-[6px] my-[10px] px-[24px] text-[18px] ${productDetails?.status === "Out of Stock" ? 'bg-orange-300' : ""} ${productDetails?.status === "Launch soon" ? 'bg-gray-300' : ""}`}>
                                    ...
                                    <div className={styles.product_menu}>
                                        <div>
                                            <button onClick={() => handleAddProductToCart(productDetails)} className={`border-2 text-white bg-[#EA5F28] border-[#EA5F28] px-4 py-2 rounded-md cursor-pointer flex gap-[5px] flex-nowrap justify-center items-center w-[200px] ${productDetails?.status === "Active" ? 'cursor-none' : 'cursor-not-allowed'}`} disabled={productDetails?.status !== "Active"}>
                            ...
                            <div className="w-fit mx-auto">
                                <div className="flex flex-col justify-center items-center mt-[25px] cursor-pointer hover:text-[#EF5D29]" onClick={() => router.push(`/category/${productDetails?.category?.id}`)}>
