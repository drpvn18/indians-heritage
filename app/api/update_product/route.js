...
export async function POST(req) {
    try {
        ...
        const product_key = body?.product_id;
        const new_product = body?.new_product;

        const idToken = await FirebaseLoginToken(authenticated_user_info?.email, authenticated_user_info?.password);

        const response = await axios.patch(`${FIREBASE_DATABASE_URL}/products/${product_key}.json?auth=${idToken}`, JSON.stringify({ ...new_product }), {
            headers: {
                'Content-Type': 'application/json',
            },
        });
