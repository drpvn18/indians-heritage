...
export async function POST(req) {
    try {
        ...
        const new_sub_category = await req.json();
        const idToken = await FirebaseLoginToken(authenticated_user_info?.email, authenticated_user_info?.password);

        const response = await axios.post(`${FIREBASE_DATABASE_URL}/sub_categories.json?auth=${idToken}`, JSON.stringify({ ...new_sub_category }), {
            headers: {
                'Content-Type': 'application/json',
            },
        });
